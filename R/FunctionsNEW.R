#' Load the dataset
#'
#' @description The function is used to download and assign a Data Frame from the file 'DATA.csv' which contains two series of observations downloaded on FRED. In particular they are the quarterly observations of US seasonally adjusted GDP per capita and Inflation, observed from 1947 to 2019.
#'
...
#'
#' @description This function can be used to upload the dataset and visualize the main plots of the package.
...
#' ggplot2 summary visualization - GDP
#'
...
ggsumpltsGDP <- function(){
  ...
  acf1 <- autoplot(acf(DATA$GDP_PERCAPITA, plot = FALSE))
  pacf1 <- autoplot(pacf(DATA$GDP_PERCAPITA, plot = FALSE))
  grid.arrange(p1, acf1, pacf1,ncol=1,nrow=3)
}


#' ggplot2 summary visualization - INFL
#'
#' @description This is a default function used to visualize the raw data, acf and pacf of the GDP data.
#' @return This function returns the raw data, the acf and the pacf.
#' @export
...
ggsumpltsINFL <- function(){
  p1 <- ggplot(DATA, aes(x=as.Date.factor(DATA$DATE), y=DATA$INFLATION))+
    geom_line(color='darkgreen', size=0.75) +
    scale_x_date(date_breaks = "8 years", date_labels = "%Y")+
    labs(title = 'USA INFLATION', x='Years', y='% change')
  acf1 <- autoplot(acf(DATA$INFLATION, plot = FALSE))
  pacf1 <- autoplot(pacf(DATA$INFLATION, plot = FALSE))
  grid.arrange(p1, acf1, pacf1, ncol=1,nrow=3)
}
...
#' ggplot2 raw data visualization
#'
